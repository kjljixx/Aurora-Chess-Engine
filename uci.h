...
void loop(chess::Board board){
  ...
  while(true){
    ...
    //if(token == "staticeval"){std::cout << (atan((board.sideToMove ? evaluation::evaluate(board).blackToMove : evaluation::evaluate(board).whiteToMove)/search::evalScaleFactor)/1.56375) << "\n";}
