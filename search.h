...
uint8_t selectEdge(Node* parent, bool isRoot, float predictedNodes){
  ...
  const float parentVisitsTerm = (isRoot ? Aurora::options["rootExplorationFactor"].value*std::log(15000) : Aurora::options["explorationFactor"].value*std::log(parent->visits));
