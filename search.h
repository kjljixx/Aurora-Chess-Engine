...
uint8_t selectEdge(Node* parent, bool isRoot){
  ...
  const float parentVisitsTerm =
      (isRoot ? Aurora::rootExplorationFactor.value : Aurora::explorationFactor.value)*
      std::log(parent->visits)*std::log(parent->visits)*std::sqrt(std::log(parent->visits));
