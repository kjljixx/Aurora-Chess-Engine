...
uint8_t selectEdge(Node* parent, bool isRoot){
  ...
  if(isRoot){
    parentVisitsTerm = Aurora::rootExplorationFactor.value*std::log(sumDiscountedVisits);
  }
  else{
    parentVisitsTerm = Aurora::explorationFactor.value*std::log(sumDiscountedVisits);
  }
...
void backpropagate(Tree& tree, float result, std::vector<std::pair<Edge*, U64>>& edges, uint8_t visits, bool forceResult, bool runFindBestMove, bool continueBackprop){
  ...
  for(int i=0; i<parent->children.size(); i++){
    if(parent->children[i].child){
      parent->children[i].child->discountedVisits *= 1.0;
    }
  }
