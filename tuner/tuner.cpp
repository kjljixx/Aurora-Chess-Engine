...
static int32_t get_phase(const texelChess::Board& board)
{
    int32_t phase = 0;

    for(uint8_t square_num = 0; square_num < 64; ++square_num)
    {
        const auto square = static_cast<texelChess::Square>(square_num);
        const auto piece = board.at(square);
        switch (piece)
        {
        case texelChess::Piece::WHITEKNIGHT:
        case texelChess::Piece::WHITEBISHOP:
        case texelChess::Piece::BLACKKNIGHT:
        case texelChess::Piece::BLACKBISHOP:
            phase += 1;
            break;
        case texelChess::Piece::WHITEROOK:
        case texelChess::Piece::BLACKROOK:
            phase += 2;
            break;
        case texelChess::Piece::WHITEQUEEN:
        case texelChess::Piece::BLACKQUEEN:
            phase += 4;
            break;
        }
    }
...
struct PvEntry
{
    array<texelChess::Move, 64> moves{};
    int32_t length = 0;
};
using pv_table_t = array<PvEntry, 64>;

static int32_t get_piece_value(const texelChess::Piece piece)
{
    switch (piece)
    {
    case texelChess::Piece::WHITEPAWN:
    case texelChess::Piece::BLACKPAWN:
        return 100;
    case texelChess::Piece::WHITEKNIGHT:
    case texelChess::Piece::BLACKKNIGHT:
        return 300;
    ...
    case texelChess::Piece::BLACKQUEEN:
        return 900;
    case texelChess::Piece::WHITEKING:
    case texelChess::Piece::BLACKKING:
    case texelChess::Piece::NONE:
        return 0;
        //throw std::runtime_error("Invalid piece for value");
    }
}

static int32_t mvv_lva(const texelChess::Board& board, const texelChess::Move move)
{
    const auto from = move.from();
    const auto to = move.to();
    const auto piece = board.at(from);
    texelChess::Piece takes;
    const auto type = move.typeOf();
    if(type == texelChess::Move::ENPASSANT)
    {
        takes = board.sideToMove() == texelChess::Color::WHITE ? texelChess::Piece::BLACKPAWN : texelChess::Piece::WHITEPAWN;
    }
    else
    {
        takes = board.at(to);
...
texelChess::Board quiescence_root(const parameters_t& parameters, const string& initial_fen)
{
    pv_table_t pv_table {};
    const auto clean_fen = cleanup_fen(initial_fen);
    auto board = texelChess::Board(clean_fen);
    auto score = quiescence(board, parameters, pv_table, -inf, inf, 0);
    if(board.sideToMove() == texelChess::Color::BLACK)
    {
        score = -score;
    }
...
static void parse_fen(const bool side_to_move_wdl, const parameters_t& parameters, vector<Entry>& entries, const string& original_fen)
{
    ...
    else
    {
        ...
        board = texelChess::Board(clean_fen);
    }
    ...
    {   
        cout << " Additional score does not work";
